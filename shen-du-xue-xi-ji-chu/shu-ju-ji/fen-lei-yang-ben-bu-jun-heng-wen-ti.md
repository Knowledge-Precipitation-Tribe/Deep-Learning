...
英文名叫做Imbalanced Data。在一般的分类学习方法中都有一个假设，就是不同类别的训练样本的数量相对平衡。
...
我们可以从不同于分类的角度去解决数据不均衡性问题，我们**可以把那些小类的样本作为异常点\(outliers\)**，因此该问题便转化为异常点检测\(anomaly detection\)与变化趋势检测问题\(change detection\)。
